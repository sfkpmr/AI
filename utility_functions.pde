...
boolean checkForCollisionsTrees(float tankX, float tankY) {
  float distX = tankX - 280;
  float distY = tankY - 230;
  float distance = sqrt( (distX*distX) + (distY*distY) );

  System.out.println(tankX + " " + tankY + " " + distance);

  if (distance <= 25 + 51) {
    System.out.println("Stoppar för träd");
    return false;
  }
  return true;
